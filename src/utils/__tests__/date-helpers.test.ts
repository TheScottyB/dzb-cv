...
describe('Date Formatting Helpers', () => {
  describe('formatUSDate', () => {
    ...
    test('handles empty or invalid inputs gracefully', () => {
      ...
      expect(formatUSDate(undefined as any)).toBe('');
      expect(formatUSDate(null as any)).toBe('');
  ...
  describe('formatFederalDateRange', () => {
    ...
    test('formats date range with Date objects', () => {
      expect(formatFederalDateRange(
        new Date('2022-01-15'), 
        new Date('2023-12-20')
      )).toBe('01/2022 – 12/2023');
      ...
      expect(formatFederalDateRange(
        new Date('2022-01-15'),
        nowDate
      )).toBe('01/2022 – Present');
    });

    test('handles empty or invalid end date as Present', () => {
      expect(formatFederalDateRange('January 2022', '')).toBe('01/2022 – Present');
      expect(formatFederalDateRange('Jan 2022', undefined as any)).toBe('01/2022 – Present');
      expect(formatFederalDateRange('1/2022', null as any)).toBe('01/2022 – Present');
    });

    test('handles empty or invalid start date gracefully', () => {
      expect(formatFederalDateRange('', 'December 2023')).toBe('');
      expect(formatFederalDateRange(undefined as any, 'Dec 2023')).toBe('');
      expect(formatFederalDateRange(null as any, '12/2023')).toBe('');
