...
async function main() {
  try {
    ...
    // Note: The original CV document was archived at:
    // "archive/assets/documents/Resume at BHGRE .docx"
    // This has been renamed to:
    // "archive/assets/documents/renamed/Resume-BHGRE.docx"
    ...
    console.log(
      '- Secondary office in Grayslake: ' +
        (bhgreEntry.address.includes('Grayslake') ? '✅ Included' : '❌ Missing')
    );
    console.log(
      '- Supervisor information: ' +
        (bhgreEntry.supervisor ? '✅ Included (' + bhgreEntry.supervisor + ')' : '❌ Missing')
    );
    console.log('- Contact permission: ' + (bhgreEntry.mayContact ? '✅ Included' : '❌ Missing'));
    console.log(
      '- Employment type: ' +
        (bhgreEntry.employmentType
          ? '✅ Included (' + bhgreEntry.employmentType + ')'
          : '❌ Missing')
    );
    console.log(
      '- Hours: ' + (bhgreEntry.hours ? '✅ Included (' + bhgreEntry.hours + ')' : '❌ Missing')
    );
    console.log(
      '- IDFPR compliance duties: ' +
        (bhgreEntry.duties.some((d) => d.includes('IDFPR')) ? '✅ Included' : '❌ Missing')
    );
    console.log(
      '- Illinois real estate law duties: ' +
        (bhgreEntry.duties.some((d) => d.includes('Illinois real estate law'))
          ? '✅ Included'
          : '❌ Missing')
    );
    console.log(
      '- Property transactions duties: ' +
        (bhgreEntry.duties.some((d) => d.includes('transactions')) ? '✅ Included' : '❌ Missing')
    );

    // All checks passed
    if (
      bhgreEntry.address.includes('Grayslake') &&
