...
const preprocessors = {
  ...
  pdf: (text) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      } else if (isShortLine && i < lines.length - 1 && !/[.!?:;]$/.test(line)) {
  ...
  linkedin: (text) => {
    return text
      .replace(
        /^(Experience|Education|Skills|Languages|Recommendations|Accomplishments)/gm,
        '## $1'
      )
...
function detectSourceType(text) {
  if (/<html|<body|<div|<p>/.test(text)) {
    ...
    text.includes('View profile badges') ||
    text.includes('Contact info') ||
    text.includes('LinkedIn member') ||
    text.includes('Skills & endorsements')
  ) {
    return 'linkedin';
  } else {
