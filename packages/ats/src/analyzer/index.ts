...
export class CVAnalyzer {
  ...
  public analyze(cv: CVData, posting: JobPosting): AnalysisResult {
    ...
    if (!posting.description && !posting.responsibilities?.length && !posting.qualifications?.length && !posting.skills?.length) {
      return {
        score: 0,
        keywordMatches: [],
        missingKeywords: [],
    ...
    const score = (keywordScore * weights.keywords) + 
  ...
  private extractCVText(cv: CVData): string {
    const parts = [
      ...
      ...cv.experience.map(exp => `${exp.position} ${exp.employer} ${exp.responsibilities.join(' ')}`),
  ...
  private extractKeywords(text: string): string[] {
    ...
    const experienceRegex = /\b(\d+\+?\s*(?:to\s+\d+\s*)?(?:-\s*\d+\s*)?years?(?:\s+(?:of|in))?\s+(?:experience|exp\.?|work(?:ing)?(?:\s+experience)?)|minimum\s+of\s+\d+\+?\s*years?|at\s+least\s+\d+\+?\s*years?|\d+\+?\s*years?\s+(?:of|in)?\s*(?:experience|exp\.?))\b/gi;
  ...
  private isStopWord(word: string): boolean {
    const stopWords = new Set([
      ...
      'by', 'as', 'be', 'this', 'that', 'we', 'our', 'you', 'your', 'their', 'from',
      'have', 'has', 'had', 'is', 'are', 'was', 'were', 'will', 'would', 'should', 'can',
      'could', 'may', 'might', 'must', 'about', 'like', 'through', 'over', 'before', 'after',
      'between', 'under', 'above', 'below', 'during', 'since', 'until', 'while', 'any',
      'all', 'each', 'every', 'some', 'more', 'most', 'other', 'such', 'no', 'not', 
      'only', 'than', 'too', 'very', 'just', 'how', 'when', 'where', 'why', 'what',
      'who', 'whom', 'which', 'if', 'then', 'else', 'so', 'amp', 'also', 'its',
      // Additional common stop words
      'job', 'work', 'skill', 'skills', 'year', 'years', 'experience', 'required', 'position',
      'team', 'role', 'looking', 'should', 'able', 'etc', 'our', 'us', 'please', 'want',
      'looking', 'help', 'using', 'good', 'great', 'excellent', 'strong'
    ]);
  ...
  private generateSuggestions(cv: CVData, posting: JobPosting, missingKeywords: string[]): string[] {
    ...
    if (uniqueMissingKeywords.length > 0) {
      suggestions.push(`Consider adding these keywords to your CV: ${uniqueMissingKeywords.join(', ')}`);
    }
    ...
    if (experienceMatch) {
      ...
      if (totalExperience < requiredYears) {
        suggestions.push(`The job requires ${requiredYears} years of experience, but your CV shows ${totalExperience} years.`);
      }
    }
