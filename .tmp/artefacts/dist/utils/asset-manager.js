...
export async function createAssetCatalog() {
    try {
        ...
        const documentMetadata = await Promise.all(documentFiles.map(file => createAssetMetadata(file)));
 ...
 */
export async function saveAssetCatalog(catalog, outputPath = join(assetsDir, 'asset-catalog.json')) {
    try {
...
export async function optimizeImage(inputPath, outputPath, quality = 85) {
    try {
        ...
        await execAsync(`magick "${inputPath}" -strip -interlace Plane -quality ${quality} "${outputPath}"`);
        // Get new file size
        const newStats = await stat(outputPath);
        const newSize = newStats.size;
        // Calculate reduction percentage
        const reductionPercentage = ((originalSize - newSize) / originalSize) * 100;
        console.log(`Image optimized and saved to ${outputPath} (reduced by ${reductionPercentage.toFixed(2)}%)`);
        return {
            path: outputPath,
            reductionPercentage
        };
...
export async function addWatermark(inputPath, watermarkText, outputPath, options = {}) {
    try {
        ...
        await execAsync(`magick "${inputPath}" -gravity ${position} -pointsize ${fontSize} ` +
