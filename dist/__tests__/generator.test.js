...
const isValidUSDateFormat = (dateStr) => {
    // Special case for "Present"
    if (dateStr === PRESENT_VALUE)
        return true;
...
describe("CV Generator", () => {
    ...
    const ensureHelpers = () => {
        ...
        if (!Handlebars.helpers.formatUSDate || !Handlebars.helpers.sortByDate) {
            ...
            if (!Handlebars.helpers.formatUSDate) {
                Handlebars.registerHelper('formatUSDate', function (input) {
                    // Handle undefined/null/empty values
                    if (!input)
                        return '';
                    // Special case for "Present" value
                    if (input === PRESENT_VALUE)
    ...
    const renderAndCheckTemplate = (template, templateData) => {
        try {
