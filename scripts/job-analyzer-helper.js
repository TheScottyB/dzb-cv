  ...
  try {
    ...
    if (lines.length > 0) {
      ...
      const isLikelyTitle =
        ...
        /\b(position|specialist|manager|director|secretary|assistant|coordinator|officer|analyst|executive|supervisor|lead|head|chief)\b/i.test(
          firstLine
        );
...
function normalizeJobAnalysis(analysis) {
  ...
  if (analysis.educationRequirements)
    normalized.educationRequirements = analysis.educationRequirements;
