...
async function main() {
  ...
  // Since we checked process.argv.length, we know this exists
  ...
  try {
    ...
    if (result.mlAnalysis) {
      ...
      console.log(`- Experience Match: ${result.mlAnalysis.categoryMatches.experience.toFixed(2)}%`);
    ...
    if (result.atsAnalysis) {
      ...
      if (result.atsAnalysis.issues.length > 0) {
        console.log('\nATS Issues:');
        result.atsAnalysis.issues.forEach((issue: { message: string }) => console.log(`- ${issue.message}`));
      }

      if (result.atsAnalysis.keywords.missing.length > 0) {
        console.log('\nMissing Keywords:');
        result.atsAnalysis.keywords.missing.forEach((keyword: string) => console.log(`- ${keyword}`));
      }
    }
