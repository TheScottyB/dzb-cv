  ...
  3. Run this command with: --use-existing-browser`)
    ...
    try {
        ...
        if (scrapedJob.location)
            console.log(`Location: ${chalk.yellow(scrapedJob.location)}`);
        console.log('\n' + chalk.green.bold('Saved Files:'));
        if (scrapedJob.htmlPath)
            console.log(`HTML: ${chalk.blue(scrapedJob.htmlPath)}`);
        if (scrapedJob.screenshotPath)
            console.log(`Screenshot: ${chalk.blue(scrapedJob.screenshotPath)}`);
...
program
    ...
    try {
        ...
        if (result.location)
            console.log(`Location: ${chalk.yellow(result.location)}`);
        if (result.jobType)
            console.log(`Job Type: ${chalk.yellow(result.jobType)}`);
        if (result.experienceLevel)
            console.log(`Experience Level: ${chalk.yellow(result.experienceLevel)}`);
        console.log('\n' + chalk.green.bold('Key Terms for CV Tailoring:'));
...
async function createTailoredCV(jobAnalysis, sector) {
    try {
        ...
        const templatePath = path.join(__dirname, "templates", sector, `${sector}-template.md`);
