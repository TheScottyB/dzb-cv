...
export default defineConfig({
  ...
  test: {
    ...
    server: {
      ...
      // Use single-threaded mode to avoid serialization issues
      workers: false
    },
    
    // Ensure we can resolve all imports
    resolve: {
      alias: [
        ...
        { find: '@dzb-cv/core', replacement: resolve(__dirname, 'packages/core/src') },
        { find: '@dzb-cv/cli', replacement: resolve(__dirname, 'packages/cli/src') },
        { find: '@dzb-cv/ui', replacement: resolve(__dirname, 'packages/ui/src') },
        { find: '@dzb-cv/pdf', replacement: resolve(__dirname, 'packages/pdf/src') },
        { find: '@dzb-cv/templates', replacement: resolve(__dirname, 'packages/templates/src') },
        { find: '@dzb-cv/types', replacement: resolve(__dirname, 'packages/types/src') },
        { find: '@dzb-cv/ats', replacement: resolve(__dirname, 'packages/ats/src') },
        { find: '@dzb-cv/config', replacement: resolve(__dirname, 'packages/config/src') },
      ],
    ...
    // Improve handling of React components
    transformMode: {
      web: [/\.[jt]sx$/]
    },
    ...
    deps: {
      registerNodeLoader: true, // Ensure non-js files are properly handled
