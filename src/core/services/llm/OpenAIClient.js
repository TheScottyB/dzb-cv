...
export class OpenAIClient {
     ...
     */
    async optimize(content, constraints = {}) {
     ...
     */
    buildDistillationPrompt(cvData, style, maxLength) {
...
Original CV:\n\n${cvText}`;
     ...
     */
    buildOptimizationPrompt(content, maxLines, maxCharactersPerLine, pageFormat) {
...
Content to optimize:\n\n${content}`;
     ...
     */
    async fallbackDistill(cvData, options) {
        ...
        // Add key skills
        if (cvData.skills.length > 0) {
            const topSkillNames = cvData.skills.slice(0, 5).map(skill => typeof skill === 'string' ? skill : skill.name);
     ...
     */
    async fallbackOptimize(content, constraints) {
        // Simulate processing delay
