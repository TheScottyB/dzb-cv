...
const createEnoentError = (syscall, path) => {
...
const mockedFs = {
    ...
    access: jest.fn(),
    rm: jest.fn(), // Assuming rm might be used implicitly or explicitly
    copyFile: jest.fn(),
    // Add others as necessary based on asset-manager.js usage
}; // Use 'as any' to handle potential type mismatches initially
...
const createMockFile = (name, type, format, size = 12345) => ({
    id: `${type}-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
    name,
    path: join(type === AssetManager.AssetType.DOCUMENT ? testDocumentsDir :
...
describe('Asset Manager', () => {
    ...
    let fs;
    let AssetManager; // Use 'any' or define a type for the imported module
    let exec;
    // Setup before all tests
    beforeAll(async () => {
        // Perform dynamic imports after mocks are configured
        fs = await import('fs/promises');
        AssetManager = await import('../artefacts/dist/utils/asset-manager.js');
    ...
    describe('File Type Detection', () => {
        test('validateFile should return valid for existing file with correct extension', async () => {
            ...
            mockedFs.stat.mockResolvedValueOnce({
                ...
                mtime: new Date(),
                birthtime: new Date(),
            }); // Use 'as any' to bypass strict Stats type checking if needed
        ...
        test('getDocumentInfo should return document information', async () => {
            ...
            // Override default mock: Simulate file exists
            mockedFs.stat.mockResolvedValueOnce(stats);
            ...
            mockedFs.stat.mockImplementation(async (path) => {
                ...
                if (path === imgPath) {
                    ...
                    mockedChildProcess.exec.mockImplementationOnce((command, callback) => {
                        ...
                        return { on: jest.fn(), stdout: { on: jest.fn() }, stderr: { on: jest.fn() } }; // Return minimal child process object
                    });
        ...
        test('saveAssetCatalog should call writeFile with correct args', async () => {
            ...
            expect(fs.writeFile).toHaveBeenCalledWith(outputPath, JSON.stringify(catalogToSave, null, 2), 'utf-8');
        });
    ...
    describe('Error Handling', () => {
        ...
        test('loadAssetCatalog should create new catalog if none exists', async () => {
            ...
            expect(fs.writeFile).toHaveBeenCalledWith(catalogPath, JSON.stringify(expectedEmptyCatalog, null, 2), 'utf-8');
        });
    }); // Close describe('Error Handling')
}); // Close top-level describe('Asset Manager')
//# sourceMappingURL=asset-manager.test.js.map