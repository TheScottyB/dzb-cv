...
function analyzeCoachingInfo(text) {
  const coachingKeywords = [
    'coach', 'mentor', 'train', 'guide', 'teach', 'instruct', 'develop', 
    'onboarding', 'education', 'workshop', 'seminar', 'class', 'session'
  ];
...
function extractTextFromParagraphs(paragraphs, _fullText = '') {
  ...
  const orgPattern = /\b[A-Z][A-Za-z]+(?: [A-Z][A-Za-z]+)*(?: (?:&|Inc\.?|LLC|Corp\.?|Company|Association|Organization))\b/g;
  const organizations = [...new Set(text.match(orgPattern) || [])];
  
  // Technology tools and systems
  const techPattern = /\b(?:software|system|platform|app|application|tool|website|portal|CRM)\b.*?(?:called|named)?\s+([A-Z][A-Za-z0-9]+|[A-Z][A-Za-z0-9]+ [A-Za-z0-9]+)/g;
...
function extractTraining(text) {
  const trainingKeywords = [
    'course', 'class', 'workshop', 'seminar', 'certification', 
    'designation', 'training', 'program', 'education', 'continuing ed'
  ];
...
function extractTools(text) {
  const toolKeywords = [
    'software', 'system', 'platform', 'tool', 'application', 'app',
    'website', 'portal', 'database', 'CRM', 'MLS'
  ];
...
async function processFile(filePath) {
  try {
    ...
    console.log(`🔍 Extracted ${entities.names.length} potential names, ${entities.organizations.length} organizations, and ${entities.tools.length} tools`);
    ...
    const analysisOutputPath = path.join(TEMP_DIR, `${path.basename(filePath, '.docx')}_analysis.json`);
...
function generateCVUpdates(analysis, fullText) {
  ...
  fs.writeFile(updatesPath, JSON.stringify({
    coachingDuties,
    leadershipSkills,
    tools: analysis.entities.tools,
    trainingSection
  }, null, 2)).catch(err => console.error(`Error writing recommendations: ${err.message}`));
