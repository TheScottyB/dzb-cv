...
★ Increased sales by 50%
      `;
            ...
            expect(analysis.issues).toContainEqual(expect.objectContaining({ type: 'COMPLEX_FORMATTING' }));
        });
...
Achievements
      `;
            ...
            expect(badAnalysis.issues).toContainEqual(expect.objectContaining({ type: 'MISSING_DATES' }));
            expect(goodAnalysis.issues).not.toContainEqual(expect.objectContaining({ type: 'MISSING_DATES' }));
        });
    });
...
• Experience
      `;
            const analysis = await analyzeATSCompatibility(content);
            expect(analysis.improvements).toEqual(expect.arrayContaining([
                expect.stringContaining('Suggestion:')
            ]));
        });
    });
});
//# sourceMappingURL=ats-compatibility.test.js.map