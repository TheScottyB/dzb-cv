...
import { analyzeJobPosting } from '../.tmp/artefacts/dist/utils/job-analyzer.js';
import { parseCvMarkdown } from '../.tmp/artefacts/dist/utils/cv-parser.js';
import { ProfileService } from '../.tmp/artefacts/dist/services/profile-service.js';
import { loadCVData } from '../.tmp/artefacts/dist/utils/helpers.js';
import { convertMarkdownToPdf } from '../.tmp/artefacts/dist/utils/pdf-generator.js';
 ...
 */
function makeATSFriendly(content) {
...
async function createTailoredCV(jobAnalysis, sector) {
    try {
        ...
        const tailoredMarkdown = `# ${cvData.personalInfo.name.full}\n\n` +
 ...
 */
async function logJobApplication(jobAnalysis, cvPath, coverLetterPath) {
    try {
        ...
        const newEntry = `## ${jobAnalysis.title} at ${jobAnalysis.company}\n\n` +
 ...
 */
async function generateSiteOptimizedCV(site, cvData, options) {
    // Normalize site name
    ...
    const siteHeader = `# ${cvData.personalInfo.name.full}\n\n` +
...
program
    ...
    try {
        ...
        if (options.file) {
            ...
            // Extract key terms
            const jobAnalyzer = await import('../.tmp/artefacts/dist/utils/job-analyzer.js');
...
// Execute the CLI
program.parse();
//# sourceMappingURL=cli-unified.js.map