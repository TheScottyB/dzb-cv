...
describe('CVService', () => {
  ...
  describe('updateCV', () => {
    ...
    it('should throw error when updating non-existent profile', async () => {
      await expect(
        service.updateCV('non-existent', { skills: [] })
  ...
  describe('generateCV', () => {
    ...
    it('should throw error for unsupported format', async () => {
      const profile = await service.createCV('Test User', testData);
      const options = {
        format: 'doc' as 'pdf' | 'markdown' // Type assertion for test
      };

      await expect(
        service.generateCV(profile.id, options)
      ).rejects.toThrow('Format doc not supported');
    });
    ...
    it('should throw error when generating for non-existent profile', async () => {
      ...
      await expect(
        service.generateCV('non-existent', options)
      ).rejects.toThrow('Profile non-existent not found');
    });
  });
});
