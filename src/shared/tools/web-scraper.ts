...
export async function scrapeJobPosting(
  ...
  const { outputDir, saveHtml, saveScreenshot, savePdf, useExistingBrowser, cdpUrl } =
    mergedOptions;
  ...
  try {
    ...
    await page.setUserAgent(
      mergedOptions.customUserAgent ?? DEFAULT_SCRAPER_OPTIONS.customUserAgent!,
    );
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, mergedOptions.waitTime ?? DEFAULT_SCRAPER_OPTIONS.waitTime!),
    );
    ...
    const htmlPath =
      saveHtml && outputDir ? path.join(outputDir, `${baseFilename}.html`) : undefined;
    const screenshotPath =
      saveScreenshot && outputDir ? path.join(outputDir, `${baseFilename}.png`) : undefined;
